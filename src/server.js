  ...
  if (!fs.existsSync(normalizedPath)) {
    ...
    if (ownerType === 'MESSAGE') {
      ...
      if (fs.existsSync(userPath)) {
        ...
        console.warn(`⚠️  File found in USER directory instead of MESSAGE: ${fileName}`);
      }
    }
    ...
    if (ownerType === 'USER' && !alternativePath) {
      ...
      if (fs.existsSync(messagePath)) {
        ...
        console.warn(`⚠️  File found in MESSAGE directory instead of USER: ${fileName}`);
      }
    }
    ...
    if (ownerType === 'ARTICLE' && !alternativePath) {
      ...
      if (fs.existsSync(userPath)) {
        alternativePath = userPath;
        console.warn(`⚠️  File found in USER directory instead of ARTICLE: ${fileName}`);
      }
    }
    ...
    if (ownerType === 'GENERAL' && !alternativePath) {
      ...
      if (fs.existsSync(userPath)) {
        alternativePath = userPath;
        console.warn(`⚠️  File found in USER directory instead of GENERAL: ${fileName}`);
      }
    }
    ...
    if (ownerType === 'USER' && !alternativePath) {
      ...
      if (fs.existsSync(generalPath)) {
        ...
        console.warn(`⚠️  File found in GENERAL directory instead of USER: ${fileName}`);
      }
    }
    ...
    const matchingFile = filesInDir.find(f => f.toLowerCase() === fileName.toLowerCase());
    if (matchingFile && matchingFile !== fileName) {
      console.warn(`⚠️  Case mismatch: Requested "${fileName}" but found "${matchingFile}"`);
